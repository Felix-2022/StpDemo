...
@end
@implementation STPBluetoothManager

+ (STPBluetoothManager *)sharedInstance
{
    static STPBluetoothManager *bluetoothManager = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^(){
        bluetoothManager = [[STPBluetoothManager alloc] init];
    });
...
- (void)setupBluetoothDelegate {
    ...
    [_bluetooth setBlockOnCentralManagerDidUpdateState:^(CBCentralManager* central) {
        ...
        if (central.state == CBCentralManagerStatePoweredOn) {
            NSLog(@"蓝牙已打开");
            self.blestate = STPBleStatePowerOn;
        }
        if (central.state == CBCentralManagerStateUnsupported) {
            NSLog(@"该设备不支持蓝牙BLE");
            self.blestate = STPBleStateUnknown;
        }
        if (central.state == CBCentralManagerStatePoweredOff) {
            NSLog(@"蓝牙已关闭");
            self.blestate = STPBleStatePoweroff;
        }
    }];
    ...
    [_bluetooth setBlockOnDiscoverToPeripherals:^(CBCentralManager* central, CBPeripheral* peripheral, NSDictionary* advertisementData, NSNumber* RSSI) {
        ...
        if (![STPBLEdataFunc isAleadyExist:blueName BLEDeviceArray:self.BLEDeviceArray]) {
            STPBLEDevice* device = [[STPBLEDevice alloc] init];
    ...
    [_bluetooth setFilterOnConnectToPeripherals:^BOOL(NSString* peripheralName, NSDictionary* advertisementData, NSNumber* RSSI) {
        ...
        if ([peripheralName hasPrefix:filterBLEname]) {
            NSLog(@"准备连接");
            self.blestate = STPBleStateConnecting;
            return YES;
        }
        return NO;
    }];
    ...
    [_bluetooth setBlockOnDiscoverServices:^(CBPeripheral* peripheral, NSError* error) {
        ...
        NSLog(@"发现服务");
        //更新蓝牙状态,进入已连接状态
        self.blestate = STPBleStateConnected;
    }];
    ...
    [_bluetooth setBlockOnDisconnect:^(CBCentralManager* central, CBPeripheral* peripheral, NSError* error) {
        ...
        if (self.APPCancelConnect) {
            //清标志位
            self.APPCancelConnect = NO;
            self.blestate         = STPBleStateDisconnect;
            NSLog(@"设备：%@--断开连接", peripheral.name);
        }
        else {
            //更新蓝牙状态,已连接状态
            self.blestate = STPBleStateReConnect;
            //添加自动回连
    ...
    [_bluetooth setBlockOnCancelScanBlock:^(CBCentralManager* centralManager) {
        ...
        if (count <= 0) {
            //更新蓝牙状态,进入无设备状态
            self.blestate = STPBleStateNoDevice;
        }
        else if (count >= 1) {
            _currentdevice = [self.BLEDeviceArray firstObject];
            self.blestate  = STPBleStateWaitToConnect;
        }
    }];
    ...
    //连接设备->
    [_bluetooth setBabyOptionsWithScanForPeripheralsWithOptions:scanForPeripheralsWithOptions
    ...
    [_bluetooth setBlockOnDidUpdateNotificationStateForCharacteristic:^(CBCharacteristic* characteristic, NSError* error) {
        ...
        if (characteristic.isNotifying) {
            NSLog(@"订阅成功");
            [self writeStructDataWithCharacteristic:self.WriteCharacteristic WithData:[PacketCommand GetDeviceInforWithSequence:self.sequence]];
...
//蓝牙状态更新调用
- (void)setBlestate:(STPBleState)blestate {
    _blestate = blestate;
    if (_bleStateBlock) {
        _bleStateBlock(blestate);
    }
    switch (blestate) {
    case STPBleStatePowerOn: {
        NSLog(@"蓝牙已打开");
        self.blestate = STPBleStateIdle;
    } break;
    case STPBleStatePoweroff: {
        NSLog(@"蓝牙已关闭");
    } break;
    case STPBleStateIdle: {
        NSLog(@"点击搜索");
    } break;
    case STPBleStateScan: {
        NSLog(@"正在扫描设备");
    } break;
    case STPBleStateCancelConnect: {
        NSLog(@"重新扫描");
    } break;
    case STPBleStateNoDevice: {
        NSLog(@"无设备,点击重新开始");
    } break;
    case STPBleStateWaitToConnect: {
        NSLog(@"等待连接");
    } break;
    case STPBleStateConnecting: {
        NSLog(@"正在连接");
    } break;
    case STPBleStateConnected:
        NSLog(@"蓝牙已连接");
        break;
    case STPBleStateDisconnect: {
        NSLog(@"蓝牙已断开,点击重新搜索");
        self.sequence  = 0;
        self.rsaobject = nil;
        self.senddata  = nil;
    ...
    case STPBleStateReConnect: {
        self.sequence  = 0;
        self.rsaobject = nil;
        self.senddata  = nil;
        self.Securtkey = nil;
    } break;
    case STPBleStateUnknown:
        NSLog(@"手机不支持蓝牙4.0");
        break;
    case STPBleStateConnecttimeout:
        NSLog(@"重连超时");
        break;
    case STPBleStateReconnecttimeout:
        NSLog(@"重连超时,点击重新开始");
        break;
    default:
        break;
    ...
    if (number > 0) {
        for (NSInteger i = 0; i < number + 1; i++) {
            if (i == number) {
                ...
                //                     NSData *data=[PacketCommand SendNegotiateData:[self.senddata subdataWithRange:NSMakeRange(0, datacount)] Sequence:self.sequence Frag:YES
                //                     TotalLength:self.senddata.length];
                //                     [self writeStructDataWithCharacteristic:_WriteCharacteristic WithData:data];
                //                     self.senddata=[self.senddata subdataWithRange:NSMakeRange(datacount, self.senddata.length-datacount)];
                //                 }];
            }
...
- (void)setOpmodeObject:(STPOpmodeObject*)object {
    ...
    [self writeStructDataWithCharacteristic:_WriteCharacteristic WithData:[PacketCommand SetStationSsid:object.wifiSSid Sequence:self.sequence Encrypt:NO WithKeyData:self.Securtkey]];
    [self writeStructDataWithCharacteristic:_WriteCharacteristic WithData:[PacketCommand SetStationPassword:object.wifiPassword Sequence:self.sequence Encrypt:NO WithKeyData:self.Securtkey]];
...
//        NSData* NegotiateData = [data subdataWithRange:NSMakeRange(4, length)];
    ...
    case Wifi_Connection_state_Report_DataSubType:  //连接状态报告
    {
        ...
        self.blestate = STPBleStateWritable;
    } break;
