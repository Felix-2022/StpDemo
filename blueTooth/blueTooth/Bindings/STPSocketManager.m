...
#import "STPSocketManager.h"
#import "GCDAsyncSocket.h"

@interface STPSocketManager()<GCDAsyncSocketDelegate>
@end

@implementation STPSocketManager {
    GCDAsyncSocket* socket;
}

+ (STPSocketManager *)defaultSocket {
    // socket只会实例化一次
    static STPSocketManager* socket=nil;
    // 保证线程安全，defaultSocket只执行一次
    static dispatch_once_t once;
    dispatch_once(&once, ^ {
        socket=[[STPSocketManager alloc] init];
    });
    return socket;
}
